...
def wyczysc_pliki(dfs: dict[int, pd.DataFrame], met: pd.DataFrame) -> dict[int, pd.DataFrame]:
    ...
    for rok, df in dfs.items():
        ...
        # Odfiltruj z listy te stacje, których nie ma w metadanych
        wspolne_kody = [kod for kod in wspolne_kody if kod in stacja_miejscowosc]

        # Teraz usuwamy unikalne kody stacji (usunie też te odfiltrowane wyżej, więc zostanie równe 95)
        for rok, df in dfs.items():
