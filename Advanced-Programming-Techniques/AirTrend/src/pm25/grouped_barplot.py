...
def policz_srednie_dzienne(df_wejsciowe: pd.DataFrame) -> pd.DataFrame:
...
def stworz_grouped_barplot(df: pd.DataFrame, lata: list) -> None:
    """
...
# stworz_grouped_barplot(polacz_dfs(dfs_obrobione))
...
def stworz_barplot_przekroczenia_woj(df_do_wykresu):
    ...
    # Dodanie tytułów
    plt.title('Liczba dni z przekroczeniem normy PM2.5 w województwach', fontsize=16, pad=20)
    ...
    plt.legend(title='Rok pomiaru', bbox_to_anchor=(1, 1), loc='upper left')
